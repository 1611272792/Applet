   ...
   public class WoBLL
    ...
    public string GetDepBonus(string CompanyId, string OpenId)
        ...
        public string GetEmpInfo(string CompanyId, string OpenId)
        {
            ...
            return state?.Rows.Count > 0 ? ResponseHandle<object>.GetResult(strSucess, "", state) : ResponseHandle<object>.GetResult(strFail, "error", null);
        }
        ...
        {
            if (dal.GetDepDeposit(CompanyId, DepID) == "success")
            {
                return ResponseHandle<object>.GetResult(strSucess, "", null);
            }
            else
            {
                return ResponseHandle<object>.GetResult(strFail, dal.GetDepDeposit(CompanyId, DepID), null);
            }
        }
        ...
        public string GetItemsBonus(string CompanyId, string OpenId)
        {
            try
            {
                ...
                {
                    return listmes != null && state.Rows.Count <= 0
                        ? ResponseHandle<object>.GetResult(strSucess, "暂无奖金项", listmes, null)
                        : ResponseHandle<object>.GetResult(strFail, "暂无信息", listmes, state);
                }
        ...
        /// <returns></returns>
        public string GetMyCompanyInfo(string OpenId)
        {
            ...
            try
            {
                DataTable dt = dal.GetMyCompanyInfo(OpenId);
                if (dt?.Rows.Count > 0)
                {
                    return ResponseHandle<object>.GetResult(strSucess, "", dt);
                }
                else
                {
                    return ResponseHandle<object>.GetResult(strFail, "暂无数据", null);
