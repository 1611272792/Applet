 ...
 public   class PinYinBLL
    {
        ...
        public string GetPinYin(string CompanyId)
        {
            ...
            {
                return ResponseHandle<object>.GetResult(strFail,"No Users", null);
            }
        }
        /// <summary>
        /// 通过字母查找人名
        ...
        /// <param name="CompanyId">公司Id</param>
        /// <returns></returns>
        public string GetDepName2(string DepName, string CompanyId)
        {
            DataTable state = dal.GetDepName(DepName.ToUpper(), CompanyId);

            if (state.Rows.Count > 0)
            {
                return ResponseHandle<object>.GetResult(strSucess, "", state.Rows[0]["DepName"].ToString() + "," + state.Rows[0]["DepId"].ToString());
            }
            else
            {
                return ResponseHandle<object>.GetResult(strFail, "No Deps");
