// pages/editposition/editposition.js
const app = getApp()
Page({
  ...
  formSubmit: function (e) {
    ...
    // }
      ...
      wx.request({
        url: that.data.durl+'/api/Sys/Position/UpdPosition',
        data: {
  ...
  onLoad: function (options) {
    ...
    var ResonContent = this.data.ResonContent;
      ...
      success: function (resc) {
        wx.request({
          ...
          success: function (resd) {
            ...
            //获取要编辑的信息
            wx.request({
              url: that.data.durl +'/api/Sys/Position/GetPosition',
              data: {
                PostId: PostId
              },
              method: "GET",
              ...
              success: function (resd) {
                ...
                if (resd.data.code == 100) {
                  if (resd.data.data[0].DepId == 0) {
                    that.setData({
                      posName: resd.data.data[0].PostName,
                      ResonContent: resd.data.data[0].Remake,
                      dropids: resd.data.data[0].PostId,
                      countryIndex: countryIndexs,
                      currentlength: resd.data.data[0].Remake.length
                    })
                  }
                  else {
                    for (var i = 0; i < that.data.countryLists.length; i++) {
                      if (that.data.countryLists[i].split(',')[1] == resd.data.data[0].DepName) {
                        countryIndexs = i;
                      }
                    }
                    that.setData({
